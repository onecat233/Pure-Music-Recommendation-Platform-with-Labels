<template>
    <div class="login-container">
      <el-card class="login-card">
        ...
        <el-form :model="form" label-width="100px">
          <el-form-item label="Name" :rules="adminnameRules">
  ...
  const adminnameRules = [
    { required: true, message: 'Adminname is required', trigger: 'blur' }
  ]
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('http://localhost:8888/admin/login/status', new URLSearchParams({
