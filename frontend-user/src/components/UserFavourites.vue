...
export default {
  ...
  try {
    const favourResponse = await axios.get('http://localhost:8888/favour/detail', {
      headers: { 'Content-Type': 'application/json' },
    });

    const favourites = favourResponse.data;
    ...
    const favouriteSongIds = favourites.map((favour) => favour.songid);

    const response = await axios.get('/song/all', {
      headers: { 'Content-Type': 'application/json' },
    });

    const favouriteSongs = response.data
      .filter((song) => favouriteSongIds.includes(song.id))
      .map((song) => ({
        ...song,
    ...
    console.error('Error fetching favourite songs:', error.message);
    alert('Failed to fetch favourite songs. Check your backend server.');
  }
},
startAddingTag(row) {
    ...
    async fetchTypesForSong(songId) {
      try {
        ...
        const topTags = types.sort((a, b) => b.agreenum - a.agreenum).slice(0, 3);
    ...
    handlePlay(song) {
      this.$emit('handlePlaySong', song.id);
    },
  ...
  try {
    if (song.favorite) {
      ...
      if (response.data.code === 1) {
        song.favorite=false;
        this.$set(this.songs,this.songs.indexOf(song),song);
      }
    } else {
